    ...
    F: std::hash::Hash + Eq + Clone + Copy + Ord,
{
    pub fn score(
        ...
        info!("[SCORER] Starting scoring: {} candidates × {} query tokens = {} operations", 
              num_candidates, num_tokens, num_candidates as u64 * num_tokens as u64);
        ...
        debug!("[SCORER] Computed avg lengths for {} fields", avg_lengths.len());
        ...
        debug!("[SCORER] Postings lookup: {} hits, {} misses ({:.1}% hit rate)", 
               postings_hits, postings_misses,
               if postings_hits + postings_misses > 0 {
                   100.0 * postings_hits as f32 / (postings_hits + postings_misses) as f32
               } else { 0.0 });
        
        debug!("[SCORER] Frequency lookup: {} hits, {} misses ({:.1}% hit rate)", 
               freq_hits, freq_misses,
        ...
        if !scores.is_empty() {
            info!("[SCORER] Completed: {} documents scored, top score: {:.4}, median: {:.4}, bottom: {:.4}", 
                  scores.len(),
    ...
    fn calculate_idf(&self, term: &str, index: &InvertedIndex<F>, total_docs: usize) -> f32 {
        ...
        debug!("[IDF] term='{}': total_df={}, fields={}, idf={:.4}", 
